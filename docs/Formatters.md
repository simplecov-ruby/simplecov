...
The **versioned canonical** lives at [`schemas/coverage-v1.3.schema.json`](../schemas/coverage-v1.3.schema.json) and
...
The current version is **1.3**. 1.1 added the optional per-test context data recorded under
...
complete context map. 1.2 added the optional `history` array (the recorded run history with this run appended). 1.3
added the optional `production` section: the window and per-file lines a
[production coverage store](Production.md) accumulated, present exactly when
[`production_coverage`](Production.md#seeing-the-cross-in-the-html-and-json-reports) names a readable store at report
time. Top-level structure:
