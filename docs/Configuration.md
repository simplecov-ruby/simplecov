...
a line with an eval-generated entry are kept (line-presence is the matcher). Templates measured with `cover_views` are
exempt: everything a template reports is eval-generated by construction, so the filter keeps all of it.
