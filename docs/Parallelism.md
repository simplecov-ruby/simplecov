...
It falls back to merging in the collating process — same report, no error — when the runtime cannot fork (Windows),
when there is only one resultset to fold, or when a worker dies.
