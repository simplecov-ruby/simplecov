...
describe('the trend column', () => {
  function trendList(trend: Parameters<typeof renderFileList>[0]['trend']) {
    return parse(
      renderFileList({
        containerId: 'g-total',
        title: 'All Files',
  ...
  test('adds an unsortable Trend header, a totals sparkline, and per-file cells', () => {
    const container = trendList({
      totals: [70, 80],
      files: new Map([[FULL, [60, 80]]])
    });

    const header = container.querySelector('th.t-trend-h')!;
    expect(header.textContent).toBe('Trend');
    expect(header.getAttribute('data-sort-key')).toBeNull();
    expect(container.querySelector('.totals-row td.t-trend svg.sparkline')).not.toBeNull();

    const cells = container.querySelectorAll('tbody td.t-trend');
    expect(cells.length).toBe(2);
    expect(cells[0].querySelector('svg.sparkline')).not.toBeNull();
    // A file the history never saw keeps its cell for column alignment,
    // empty rather than misleading.
    expect(cells[1].innerHTML).toBe('');
  });

  test('renders exactly as before when the report embeds no history', () => {
    const container = trendList(undefined);
    expect(container.querySelector('.t-trend-h')).toBeNull();
    expect(container.querySelector('td.t-trend')).toBeNull();
  });
});
