...
RSpec.describe SimpleCov::UselessResultsRemover do
  ...
  it "drops the gem file path from the result set" do
  ...
  describe ".root_regex" do
    ...
    context "when the root changes" do
      ...
      it "matches under the new root" do
        expect(rebuilt.match?("#{two}/lib/a.rb")).to be(true)
      end

      it "no longer matches under the old one" do
    ...
    context "with a root that looks like a pattern" do
      before do
        described_class.instance_variable_set(:@root_regex_root, nil)
        allow(SimpleCov).to receive(:root).and_return("/one+two")
      end

      it "matches the path it spells" do
        expect(described_class.root_regex.match?("/one+two/lib/a.rb")).to be(true)
      end

      it "matches nothing the pattern would have" do
