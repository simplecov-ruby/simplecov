...
RSpec.describe SimpleCov::Filter, mutant_expression: ["SimpleCov::Filter*", "SimpleCov::Configuration*"] do
  ...
  it "anchors a directory-only filter to a segment boundary" do
    expect(SimpleCov::StringFilter.new("lib/")).not_to be_matches nested_library_file
  end

  it "matches a directory-only filter against the segment it names" do
    expect(SimpleCov::StringFilter.new("mylib/")).to be_matches nested_library_file
  end

  it "leaves a bare segment unrelaxed, so it matches no segment that merely ends with it" do
    expect(SimpleCov::StringFilter.new("lib")).not_to be_matches nested_library_file
  end

  it "relaxes to a within-segment match for a bare filename" do
    expect(SimpleCov::StringFilter.new("oo.rb")).to be_matches nested_library_file
  end

  def nested_library_file
    SimpleCov::SourceFile.new(File.join(SimpleCov.root, "mylib/foo.rb"), [nil, 1, 1])
  end
  ...
  it "doesn't match a parent directory with a new SimpleCov::StringFilter" do
    parent_dir_name = File.basename(File.expand_path("../..", __dir__))
  ...
  it "matches a new SimpleCov::ArrayFilter when a custom class that returns true is passed as array" do
    filter = Class.new(SimpleCov::Filter) { def matches?(_source_file) = true }.new(nil)
  ...
  it "doesn't match a new SimpleCov::ArrayFilter when a custom class that returns false is passed as array" do
    filter = Class.new(SimpleCov::Filter) { def matches?(_source_file) = false }.new(nil)
  ...
  describe "#remove_filter" do
    ...
    it "keeps the filters a removed string filter did not match" do
      SimpleCov.skip "spec"
      SimpleCov.skip "lib"
    ...
    it "keeps the filters a removed regex filter did not match" do
      SimpleCov.skip(/\A\..*/)
      SimpleCov.skip(/\Aapp/)
      SimpleCov.remove_filter(/\A\..*/)
    ...
    it "keeps the filter chain when nothing matches" do
      SimpleCov.skip "spec"
      SimpleCov.remove_filter("nope")
  ...
  describe ".build_filter" do
    ...
    it "builds an array filter from a list" do
    ...
    it "builds a regex filter from a regex, whose own type is neither" do
      expect(described_class.build_filter(/re/)).to be_a(SimpleCov::RegexFilter)
    end

    it "builds a block filter from a lambda, whose own type is neither" do
  ...
  describe ".filter_classes_by_argument_type" do
    ...
    it "names a class for each type of argument" do
      expect(described_class.send(:filter_classes_by_argument_type))
        .to eq(String => SimpleCov::StringFilter, Regexp => SimpleCov::RegexFilter,
          Array => SimpleCov::ArrayFilter, Proc => SimpleCov::BlockFilter)
    end

    it "answers a frozen table" do
      expect(described_class.send(:filter_classes_by_argument_type)).to be_frozen
    end

    it "answers the same table each time" do
  ...
  describe "#path_only?" do
    it "is true for a string filter, which only looks at the path" do
      expect(SimpleCov::StringFilter.new("foo")).to be_path_only
    end

    it "is true for a regex filter, which only looks at the path" do
      expect(SimpleCov::RegexFilter.new(/foo/)).to be_path_only
    end

    it "is true for a glob filter, which only looks at the path" do
    ...
    it "is true for an array filter when every component is" do
      expect(SimpleCov::ArrayFilter.new(["foo", /bar/])).to be_path_only
    end

    it "is false for an array filter when a component is not" do
