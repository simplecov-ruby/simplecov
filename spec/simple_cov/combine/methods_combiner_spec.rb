...
RSpec.describe SimpleCov::Combine::MethodsCombiner do
  describe ".combine" do
    it "sums coverage for matching method keys" do
      coverage_a = {'["A", :method1, 2, 2, 5, 5]' => 3, '["A", :method2, 9, 2, 11, 5]' => 0}
      coverage_b = {'["A", :method1, 2, 2, 5, 5]' => 2, '["A", :method2, 9, 2, 11, 5]' => 1}

      expect(described_class.combine(coverage_a, coverage_b))
        .to eq('["A", :method1, 2, 2, 5, 5]' => 5, '["A", :method2, 9, 2, 11, 5]' => 1)
    end
    ...
    it "preserves methods unique to one side" do
      ...
      expect(described_class.combine(coverage_a, coverage_b))
        .to eq('["A", :method1, 2, 2, 5, 5]' => 1, '["B", :method2, 9, 2, 11, 5]' => 2)
    end
    ...
    it "sums duplicated identities arriving within one side" do
      coverage_a = {'["ClassA", :method_added, 18, 55, 22, 9]' => 6, '["ModuleB", :method_added, 18, 55, 22, 9]' => 0}
  ...
  describe ".absorb" do
    ...
    it "leaves an existing target empty when there is no coverage" do
      target = {}
    ...
    it "sums a method it has already absorbed" do
      target = described_class.absorb(nil, key => 3)
      described_class.absorb(target, key => 4)
  ...
  describe ".identities" do
    ...
    it "gives two keys of the same location one id" do
      identities = described_class.identities

      expect(identities[["A", :method1, 2, 3, 4, 5]]).to eq(identities[["B", :method2, 2, 3, 4, 5]])
    end

    it "gives keys of different locations different ids" do
      identities = described_class.identities
