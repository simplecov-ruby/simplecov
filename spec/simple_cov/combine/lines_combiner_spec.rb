...
RSpec.describe SimpleCov::Combine::LinesCombiner do
  describe ".merge_into" do
    ...
    it "folds the source into that same array" do
      target = [3, 7]
      described_class.merge_into(target, [4, 2])
    ...
    it "answers the source's counts when there is no target yet" do
      source = [1, nil]
      expect(described_class.merge_into(nil, source)).to eq(source)
    end

    it "duplicates the source rather than answering it" do
      source = [1, nil]
  ...
  describe ".sum_into" do
    it "answers the target it folded into" do
      target = [3, nil, 7]

      expect(described_class.sum_into(target, [5, 4, 2], 3)).to be(target)
    end

    it "sums each position into that same array" do
      target = [3, nil, 7]
      described_class.sum_into(target, [5, 4, 2], 3)
