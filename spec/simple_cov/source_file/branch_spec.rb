# frozen_string_literal: true

require "helper"

RSpec.describe SimpleCov::SourceFile::Branch do
  let(:if_branch) do
    described_class.new(start_line: 1, end_line: 3, coverage: 0, inline: false, type: :then)
  end

  let(:else_branch) do
    described_class.new(start_line: 1, end_line: 3, coverage: 0, inline: false, type: :else)
  end

  context "when a source branch if..else" do
    it "reports the then branch under its own type" do
      expect(if_branch.report).to eq([:then, 0])
    end

    it "reports the else branch under its own type" do
      expect(else_branch.report).to eq([:else, 0])
    end
  end

  it "keeps every attribute it was built with" do
    branch = described_class.new(start_line: 5, end_line: 8, coverage: 3, inline: false, type: :then)

    expect(branch).to have_attributes(start_line: 5, end_line: 8, coverage: 3, type: :then)
  end

  it "is neither inline nor skipped when it was built neither" do
    branch = described_class.new(start_line: 5, end_line: 8, coverage: 3, inline: false, type: :then)

    expect(branch).to have_attributes(inline?: false, skipped?: false)
  end

  it "is inline when it was built inline" do
    branch = described_class.new(start_line: 5, end_line: 5, coverage: 1, inline: true, type: :then)

    expect(branch.inline?).to be true
  end

  describe "#report_line" do
    it "reports an inline branch on its own first line" do
      branch = described_class.new(start_line: 5, end_line: 5, coverage: 1, inline: true, type: :then)

      expect(branch.report_line).to eq(5)
    end

    it "reports a block branch on the line above its first" do
      branch = described_class.new(start_line: 5, end_line: 8, coverage: 1, inline: false, type: :then)

      expect(branch.report_line).to eq(4)
    end
  end

  context "when A source branch with coverage" do
    let(:covered_branch) do
      described_class.new(start_line: 1, end_line: 3, coverage: 1, inline: false, type: :then)
    end

    it "is covered" do
      expect(covered_branch).to be_covered
    end

    it "isn't covered if skipped" do
      covered_branch.skipped!

      expect(covered_branch).not_to be_covered
    end

    it "isn't missed if skipped" do
      covered_branch.skipped!

      expect(covered_branch).not_to be_missed
    end

    it "is not missed" do
      expect(covered_branch).not_to be_missed
    end
  end

  context "when a source branch without coverage" do
    let(:uncovered_branch) do
      described_class.new(start_line: 1, end_line: 3, coverage: 0, inline: false, type: :then)
    end

    it "isn't covered" do
      expect(uncovered_branch).not_to be_covered
    end

    it "is missed" do
      expect(uncovered_branch).to be_missed
    end

    it "isn't covered if skipped" do
      uncovered_branch.skipped!

      expect(uncovered_branch).not_to be_covered
    end

    it "isn't missed if skipped" do
      uncovered_branch.skipped!

      expect(uncovered_branch).not_to be_missed
    end
  end

  describe "skipping lines" do
    subject(:branch) { described_class.new(start_line: 5, end_line: 7, coverage: 0, inline: false, type: :then) }

    it "isn't skipped by default" do
      expect(branch).not_to be_skipped
    end

    it "can be skipped" do
      branch.skipped!
      expect(branch).to be_skipped
    end
  end

  describe "#overlaps_with?" do
    subject(:branch) { described_class.new(start_line: 5, end_line: 7, coverage: 0, inline: false, type: :then) }

    it "doesn't overlap with a range beyond its lines" do
      expect(branch.overlaps_with?(8..10)).to be false
    end

    it "doesn't overlap with a range before its lines" do
      expect(branch.overlaps_with?(3..4)).to be false
    end

    it "overlaps with a range that fully includes everything" do
      expect(branch.overlaps_with?(1..100)).to be true
    end

    it "overlaps with a range that exactly includes it" do
      expect(branch.overlaps_with?(5..7)).to be true
    end

    it "overlaps with a range that partially includes its beginning" do
      expect(branch.overlaps_with?(1..5)).to be true
    end

    it "overlaps with a range that partially includes its end" do
      expect(branch.overlaps_with?(7..10)).to be true
    end

    it "overlaps with a range that pends in its middle" do
      expect(branch.overlaps_with?(1..6)).to be true
    end
  end
end
