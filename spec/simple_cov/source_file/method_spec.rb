...
RSpec.describe SimpleCov::SourceFile::Method do
  ...
  it "keeps the names in the coverage key it was built from" do
    method = described_class.new(source_file, ["A", :method1, 2, 4, 5, 7], 1)

    expect(method).to have_attributes(class_name: "A", method_name: :method1)
  end

  it "keeps the positions in the coverage key it was built from" do
    method = described_class.new(source_file, ["A", :method1, 2, 4, 5, 7], 1)

    expect(method).to have_attributes(start_line: 2, start_col: 4, end_line: 5, end_col: 7)
  end

  it "keeps the source file it was built with" do
    expect(the_method.source_file).to be(source_file)
  end

  it "keeps the coverage it was built with" do
  ...
  describe "#skipped!" do
    it "marks the method as skipped regardless of its line coverage" do
      ...
      expect(the_method).to have_attributes(skipped?: true, covered?: false, missed?: false)
    end
  ...
  describe "#overlaps_with?" do
    it "is true for a range inside the method's own" do
      expect(the_method.overlaps_with?(3..4)).to be true
    end

    it "is true for a range that meets the method's first line" do
      expect(the_method.overlaps_with?(1..2)).to be true
    end

    it "is true for a range that meets the method's last line" do
      expect(the_method.overlaps_with?(5..7)).to be true
    end

    it "is false for a range that starts after the method's last line" do
      expect(the_method.overlaps_with?(6..10)).to be false
    end

    it "is false for a range that ends before the method's first line" do
