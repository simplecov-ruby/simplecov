...
RSpec.describe SimpleCov::ViewCoverage do
  ...
  describe ".compile_unrendered" do
    it "answers the templates the run never rendered" do
    ...
    it "answers nothing when the project didn't ask for view coverage" do
      allow(SimpleCov).to receive(:view_coverage?).and_return(false)

      expect(described_class.compile_unrendered).to be_empty
    end

    it "compiles nothing when the project didn't ask for view coverage" do
      allow(SimpleCov).to receive(:view_coverage?).and_return(false)
      described_class.compile_unrendered
  ...
  describe ".discover" do
    let(:path_only_filter) { SimpleCov::StringFilter.new("nothing") }
    let(:needs_source_filter) { SimpleCov::BlockFilter.new(->(_source_file) { true }) }

    it "rejects by the filters that need no coverage to decide" do
      allow(SimpleCov).to receive(:filters).and_return([path_only_filter, needs_source_filter])
      ...
      expect(SimpleCov::UnloadedFileInjector)
        .to have_received(:discover).with(anything, root: root, reject: [path_only_filter])
    end
  ...
  describe ".measured_paths" do
    it "answers the measured paths" do
      expect(described_class.measured_paths).to eq(Set[rendered])
    end

    it "answers them as a Set" do
