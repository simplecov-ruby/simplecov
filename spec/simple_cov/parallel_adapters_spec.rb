...
RSpec.describe SimpleCov::ParallelAdapters do
  ...
  def parallel_env(worker, groups)
    ENV["TEST_ENV_NUMBER"] = worker
    ENV["PARALLEL_TEST_GROUPS"] = groups
  end

  def with_parallel_tests(setting)
    previous = SimpleCov.parallel_tests
    SimpleCov.parallel_tests setting
    yield
  ensure
  ...
  describe ".register" do
    ...
    it "selects the shipped adapter until a custom one is registered" do
    ...
    it "clears the memoized current selection" do
      ...
      described_class.register(active_adapter)
      expect(described_class.current).to be(active_adapter)
    end
  end

  describe ".current" do
    it "is nil when no adapter is active" do
      parallel_env(nil, nil)
  ...
  describe SimpleCov::ParallelAdapters::Base do
    ...
    it "wait_for_siblings raises nothing" do
      expect { described_class.wait_for_siblings }.not_to raise_error
    end

    it "wait_for_siblings is a no-op, answering nil" do
  ...
  describe SimpleCov::ParallelAdapters::GenericAdapter do
    ...
    describe ".active?" do
      ...
      it "is false when SimpleCov.parallel_tests is false" do
        ...
        with_parallel_tests(false) { expect(described_class.active?).to be false }
      end
    end
  ...
  describe SimpleCov::ParallelAdapters::ParallelTestsAdapter do
    ...
    describe ".first_worker?" do
      it "delegates a true first_process? to ParallelTests" do
        stub_const("ParallelTests", Class.new { def self.first_process? = true })
        expect(described_class.first_worker?).to be true
      end

      it "delegates a false first_process? to ParallelTests" do
        stub_const("ParallelTests", Class.new { def self.first_process? = false })
        expect(described_class.first_worker?).to be false
      end
    end
