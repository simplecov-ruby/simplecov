...
RSpec.describe SimpleCov::ExitCodes do
  describe SimpleCov::ExitCodes::Check, mutant_expression: "SimpleCov::ExitCodes::Check*" do
    ...
    it "answers its violations as values" do
      expect(check.violations).to eq([{name: "first"}, {name: "second"}])
    end

    it "computes them once however often they are asked for" do
      2.times { check.violations }
    ...
    it "prints exactly its rendered lines, through the enforcement printer, in order" do
      printed = []
      allow(SimpleCov::ExitCodes).to receive(:print_error) { |line| printed << line }

      check.report

      expect(printed).to eq(["line for first", "line for second"])
    end
  end
