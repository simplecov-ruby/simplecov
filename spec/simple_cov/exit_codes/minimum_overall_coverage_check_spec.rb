...
RSpec.describe SimpleCov::ExitCodes::MinimumOverallCoverageCheck,
  ...
  describe "#report" do
    ...
    it "heads the list of files with the criterion" do
      expect(output).to include("Lowest-coverage files (line):")
    end

    it "lists the worst-covered file" do
      expect(output).to include("lib/worst.rb")
    end

    it "lists the next-worst file after it" do
      expect(output.index("lib/middle.rb")).to be > output.index("lib/worst.rb")
    end

    it "leaves fully covered files out of the list" do
      allow(result).to receive(:files).and_return(files + [file_double("lib/covered.rb", line: 100.0)])
    ...
    it "skips files that lack stats for the violated criterion" do
      ...
      allow(result).to receive(:files).and_return(files + [missing])
