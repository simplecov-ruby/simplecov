...
RSpec.describe SimpleCov::StaticCoverageExtractor do
  ...
  describe "#begin_modifier_loop?" do
    ...
    it "asks a node that can answer, and answers true where it says so" do
      expect(visitor.send(:begin_modifier_loop?, double(begin_modifier?: true))).to be(true)
    end

    it "asks a node that can answer, and answers false where it says so" do
    ...
    context "when Prism is available", if: described_class.available? do
      ...
      describe "branch enumeration" do
        ...
        it "walks a call node that cannot say whether it is safe navigation" do
          ...
          visitor.visit_call_node(childless_call_node)
        ...
        context "with a constant-folded condition" do
          ...
          it "matches the running compiler's treatment of `__FILE__`" do
            folded = SimpleCov::StaticCoverageExtractor::ConditionFolding::FOLDS_SOURCE_FILE
      ...
      describe "container eliminability predicates" do
        ...
        describe "the parse.y era conventions" do
          ...
          it "asks even so that no call be among them" do
            stub_era("CONTAINER_CONTENTS_NEED_STATIC_LITERALS", false)
          ...
          it "counts a scalar literal among them from 3.4 on" do
            stub_era("CONTAINER_CONTENTS_NEED_STATIC_LITERALS", true)
  ...
  describe "the modern branch conventions", if: described_class.available? do
    ...
    it "emits no branch for a one-line pattern match" do
      ...
      expect(sources.map { |source| described_class.call(source)["branches"] }).to all(be_empty)
    end
  end
  ...
  describe "the legacy branch conventions", if: described_class.available? do
    ...
    def conventions_marking(node)
      Object.new.extend(described_class::LocationConventions).tap do |context|
    ...
    it "consults the map where the visitor built one" do
      ...
      expect(conventions_marking(node).send(:value_position?, node)).to be true
    end

    it "answers no for a node the map does not hold" do
      expect(conventions_marking(Object.new).send(:value_position?, Object.new)).to be false
    end
  end
  ...
  describe SimpleCov::StaticCoverageExtractor::ValuePositions,
    ...
    it "forwards tail position into both arms of an if and an unless" do
      ...
      expect(sources.map { |source| marked_leaves(source) }).to all(eq([":else", ":then"]))
    end
    ...
    it "discards tail position into a loop body, a block, and an assignment" do
      ...
      expect(sources.map { |source| marked_leaves(source) }).to all(be_empty)
    end
    ...
    it "marks a program whose only statement it is" do
    ...
    it "answers an identity set, so equal-looking nodes stay distinct" do
      expect(same_leaf_positions).to be_compare_by_identity
    end

    it "marks every node the set holds" do
      expect(same_leaf_positions.values.uniq).to eq([true])
    end

    it "raises nothing over anything that is not a node" do
      expect { described_class.mark(nil, true, {}) }.not_to raise_error
    end

    it "marks nothing for anything that is not a node" do
