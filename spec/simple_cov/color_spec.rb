...
RSpec.describe SimpleCov::Color do
  describe ".for_percent" do
    it "returns :green at the green threshold" do
      expect(described_class.for_percent(90.0)).to eq(:green)
    end

    it "returns :green just short of full coverage" do
      expect(described_class.for_percent(99.99)).to eq(:green)
    end

    it "returns :green at full coverage" do
      expect(described_class.for_percent(100.0)).to eq(:green)
    end

    it "returns :yellow at the yellow threshold" do
    ...
    it "returns :yellow just short of the green threshold" do
      expect(described_class.for_percent(89.99)).to eq(:yellow)
    end

    it "returns :red at no coverage at all" do
      expect(described_class.for_percent(0.0)).to eq(:red)
    end

    it "returns :red just short of the yellow threshold" do
  ...
  describe ".enabled?" do
    ...
    def stream_with_tty(tty)
      stream = StringIO.new
      allow(stream).to receive(:tty?).and_return(tty)
      stream
    end
    ...
    it "falls back to a stderr that is a TTY when neither env var is set" do
      allow($stderr).to receive(:tty?).and_return(true)

      expect(described_class).to be_enabled
    end

    it "falls back to a stderr that is not a TTY when neither env var is set" do
      allow($stderr).to receive(:tty?).and_return(false)

      expect(described_class).not_to be_enabled
    end

    it "checks a given stream that is a TTY" do
      expect(described_class.enabled?(stream_with_tty(true))).to be true
    end

    it "checks a given stream that is not a TTY" do
      expect(described_class.enabled?(stream_with_tty(false))).to be false
    end
    ...
    context "when SimpleCov carries no color config (standalone CLI)" do
      before do
        allow(SimpleCov).to receive(:respond_to?).and_call_original
        allow(SimpleCov).to receive(:respond_to?).with(:color).and_return(false)
      end

      it "falls through to the stream's own answer" do
        expect(described_class.enabled?(stream_with_tty(true))).to be true
      end

      it "falls through to NO_COLOR" do
        ENV["NO_COLOR"] = "1"

        expect(described_class.enabled?(stream_with_tty(true))).to be false
      end
