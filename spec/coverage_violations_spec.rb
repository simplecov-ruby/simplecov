...
RSpec.describe SimpleCov::CoverageViolations, mutant_expression: "SimpleCov::CoverageViolations*" do
  ...
  describe "violation shapes" do
    ...
    it "looks up oneshot line thresholds in the line bucket they fold into" do
      totals = instance_double(SimpleCov::Result, coverage_statistics: {line: line_stats})
    ...
    it "reports the overall minimum and maximum violations with their own percents" do
      totals = instance_double(SimpleCov::Result, coverage_statistics: {line: line_stats})
    ...
    it "asks the baseline about this file's own path and criterion" do
      ...
      expect(described_class.minimum_by_file(result, {line: 100, branch: 100}, baseline: baseline)
                            .map { |violation| violation[:criterion] }).to eq([:branch])
      expect(described_class.maximum_missed_by_file(result, {line: 0, branch: 0}, baseline: baseline)
                            .map { |violation| violation[:criterion] }).to eq([:branch])
    end
  ...
  describe ".baseline" do
    ...
    it "rounds the percent it reports the way every other check does" do
      ...
      violation = described_class.baseline(instance_double(SimpleCov::Result, files: [uneven]), baseline).first
      expect(violation[:actual]).to eq(33.33)
    end
    ...
    it "keeps checking the files that follow one the baseline has no entry for" do
      ...
      pair = instance_double(SimpleCov::Result, files: [file, other])
  ...
  describe "drop baselines" do
    ...
    context "with the last run" do
      ...
      it "treats a result the file never recorded as nothing to compare against" do
