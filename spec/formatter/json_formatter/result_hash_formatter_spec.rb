...
RSpec.describe SimpleCov::Formatter::JSONFormatter::ResultHashFormatter,
  ...
  it "carries the schema pointer and the sections a report always has" do
  ...
  it "describes the run in meta" do
    expect(document.fetch(:meta)).to eq(
  ...
  it "totals only the criteria the run measured" do
    expect(document.fetch(:total)).to eq(
      lines: {covered: 2, missed: 1, omitted: 17, total: 3, percent: 200 / 3.0, strength: 2 / 3.0}
    )
  end
  ...
  it "embeds the source by default and omits it on request" do
  ...
  it "carries the recorded context list and each file's bitmaps" do
    map = SimpleCov::ContextMap.new
    map.record("a test", {sample_path => 0b0110})
    contextual = described_class.format(result_with_contexts(map))

    expect(contextual.fetch(:contexts)).to eq(["a test"])
    expect(contextual.fetch(:coverage).fetch(project_filename)).to include(contexts: {"0" => "6"})
  end
