    ...
    module Completions
      ...
      module_function
        ...
        def fish_option(name, option)
          ...
          parts << "-s #{(_ = option[:short]).delete_prefix('-')}" if option[:short]
          parts << "-l #{option[:long].delete_prefix('--')}"
          parts << "-r" if option[:arg]
          parts << "-d #{fish_quote(option[:desc])}"
          parts.join(" ")
        end
        ...
        def bash_cases(options)
          ...
          options.each do |name, list|
            ...
            words = list.flat_map { |option| [option[:short], option[:long]] }.compact.join(" ")
        ...
        def zsh_cases(options)
          options.collect do |name, list|
            specs = list.flat_map { |option| zsh_specs(option) }
            specs.unshift("'1:shell:(fish bash zsh)'") if name == "completions"
            "    #{name}) _arguments #{specs.join(' ')};;"
          end
        end
        ...
        def zsh_specs(option)
          desc = "[#{zsh_quote(option[:desc].delete('[]'))}]"
          specs = [] #: Array[String]
          specs << "'#{option[:short]}#{desc}'" if option[:short]
          specs << "'#{option[:long]}#{'=' if option[:arg]}#{desc}'"
        end
